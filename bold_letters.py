...
def main(side,img_index,testing):
    ...
    kernel_uniform_tiny = np.ones((2,2),np.uint8)
    ...
    dst_hor = cv2.erode(dst_hor,kernel_vert_short,iterations=5)
    ...
    dst_vert = cv2.erode(dst_vert,kernel_hor_short,iterations=5)
    ...
    d_im = cv2.dilate(no_lines, kernel_uniform, iterations=1)
    # apply a filter to remove speckle
    d_im = median(d_im, disk(2))
    e_im = cv2.erode(d_im, kernel_uniform, iterations=1)
    # try and remove dashed vertical lines with a little dilation and then erosion
    #d_im = cv2.dilate(e_im, kernel_spec2, iterations=1)
    #e_im = cv2.erode(d_im, kernel_uniform, iterations=1)
    
    # convert to binary
    no_lines = cv2.bitwise_not(e_im)


    # plot if troubleshooting
    if testing == 1:
